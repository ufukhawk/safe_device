...
#if __has_include(<DTTJailbreakDetection/DTTJailbreakDetection.h>)
#import <DTTJailbreakDetection/DTTJailbreakDetection.h>
#define SAFE_DEVICE_HAS_DTT_JAILBREAK_DETECTION 1
#else
#define SAFE_DEVICE_HAS_DTT_JAILBREAK_DETECTION 0
#endif
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
  FlutterMethodChannel *channel =
      [FlutterMethodChannel methodChannelWithName:@"safe_device"
...
- (void)handleMethodCall:(FlutterMethodCall *)call
  ...
  if ([@"getPlatformVersion" isEqualToString:call.method]) {
    ...
    // For now we have returned if device is Jail Broken or if it's not real
    // device. There is no strong detection of Mock location in iOS
    result([NSNumber
        numberWithBool:([self isJailBroken] || ![self isRealDevice])]);
...
- (BOOL)hasLegitimateEnvironmentVariables {
  ...
  NSArray *legitimatePaths = @[
    @"/System/Library/PrivateFrameworks/", @"/usr/lib/system/",
    @"/System/Library/Frameworks/", @"/Developer/", @"/Applications/Xcode.app/",
    @"libBacktraceRecording.dylib", @"libMainThreadChecker.dylib",
    @"IDEBundleInjection"
  ];
...
// Mac Catalyst apps run on macOS — not jailbroken
  ...
  BOOL dttResult = NO;
#if SAFE_DEVICE_HAS_DTT_JAILBREAK_DETECTION
  ...
  // If DTT or custom detection says jailbroken, double-check environment
  // variables
